const Loading = () => {
    return ( 
        <div className="bg-blue-100 min-w-screen py-40 min-h-screen overflow-x-hidden flex flex-col items-center">
            <div className="bg-white m-2 w-4/5 p-5 rounded drop-shadow">
                <div className='flex justify-between'>
                <div className='flex w-2/6 bg-gray-50 rounded-full'>
                    <div className='w-20 bg-gray-300 animate-pulse h-20 rounded '><p className='text-black text-center'></p> </div>
                    <span className='mt-3 ml-4 bg-gray-200 inline-block h-3 rounded w-28 animate-pulse'></span>
                </div>
                <div className='w-2/5 bg-gray-200 p-1 rounded-xl drop-shadow-xl animate-pulse'>
                    <span className='mt-3 ml-3 bg-white inline-block h-2 rounded w-10/12'></span>
                    <span className='mt-3 ml-3 bg-white inline-block h-2 rounded w-4/12'></span>
                </div>
                <div className='bg-white h-full'>

                <div className='flex items-center animate-pulse'>
                     <span className='h-3 w-3 bg-gray-700 rounded-full inline-block border-1 border-gray-300 border-dotted shadow-xl pt-2 drop-shadow-lg'></span>
                     <div className='ml-5 text-sm bg-gray-100 rounded p-1 font-[500]  drop-shadow-sm w-14'><span className='mt-3 ml-3 bg-white inline-block h-2 rounded w-9'></span></div>
                 </div> 

                 <div className='flex items-center relative'>
                     <div className='text-sm text-white animate-pulse mt-5 bg-gray-500 p-1 rounded drop-shadow-lg '><span className='mt-3 ml-3 bg-white inline-block h-2 rounded w-9'></span></div>
                 </div>

                </div>
            </div>
            </div>
            <div className='mt-20 bg-gray-100 w-11/12 rounded-xl drop-shadow-lg p-5 m-5'>
            <span className=' p-2 rounded bg-gray-400 h-2 w-5/12 animate-pulse inline-block'></span>
                <ul>
                    <div className='bg-white rounded drop-shadow m-5 p-5'>
                       <span className=' rounded bg-gray-300 h-2 w-5/12 animate-pulse inline-block'></span>
                            {
                                [1,2,3].map(ele => <li key={ele} className='bg-gray-50 rounded-full p-4 my-3 drop-shadow'>
                                <span className='rounded bg-gray-200 h-2 w-5/12 animate-pulse inline-block'></span>
                            </li>)
                            }
                        </div>
                </ul>
            </div>
        </div>
     );
}
 
export default Loading;